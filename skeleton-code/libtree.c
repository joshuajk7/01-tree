...
tree_print_recurse(struct fileinfo finfo)
{
  ...
  if (!S_ISDIR(finfo.st.st_mode) && putchar('\n') == EOF) goto exit;
...
print_path_info(struct fileinfo finfo)
{
  ...
  if (opts.user) {
    ...
    if (printf("%c%s", sep, pwd->pw_name) < 0) goto exit; /* TODO */
    sep = ' ';
  }
  if (opts.group) {
    /*  Hint: getgrgid(3) */
    struct group *grp = getgrgid(finfo.st.st_gid);
    if (grp == NULL) goto exit;
    if (printf("%c%s", sep, grp->gr_name) < 0) goto exit; /* TODO */
    sep = ' ';
  }
  if (opts.size) {
    /*  Hint: stat.h(0p) */
...
filecmp(void const *_lhs, void const *_rhs)
{
  ...
  switch (opts.sort) {
    case NONE:
      ...
      break;
    case ALPHA:
      retval = strcmp(lhs->path, rhs->path);
      break; /* TODO */
    case RALPHA:
